...
public class ProfileController {
    ...
    public AllOutProfile getProfileBasic(@RequestHeader("Authorization") String authorization) {
    ...
    @PostMapping("/profileBasic/add_friend")
    @ResponseStatus(HttpStatus.OK)
    public void addFriendBasic(@RequestHeader("Authorization") String authorization, @RequestParam(name="slave") String slave) {
        User user = userService.getByName(Authorization.parseBasicUsername(authorization));
        // null pointer
        profileService.addFriend(user.getUsername(), slave);
    }
    ...
    @PostMapping("/profileBasic/update_coords")
    @ResponseStatus(HttpStatus.OK)
    public void updateCoordsBasic(@RequestHeader("Authorization") String authorization, @RequestBody User user) {
        User userDb = userService.getByName(Authorization.parseBasicUsername(authorization));
        userService.updateCoords(userDb.getUsername(), user);
    }
    ...
    @PostMapping("/profileBasic/update")
    @ResponseStatus(HttpStatus.OK)
    public void updateBasic(@RequestHeader("Authorization") String authorization, @RequestBody User user) {
        User userDb = userService.getByName(Authorization.parseBasicUsername(authorization));
        userService.update(userDb.getUsername(), user);
    }
}
